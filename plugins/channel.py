...
async def send_msg(bot, filename, caption):
    ...
    logger.info("Preparing send_msg for %s", filename)
    ...
    try:
        ...
        logger.debug("DB upsert for '%s', is_new=%s", filename, is_new)
    ...
    try:
        ...
        logger.debug("IMDB details for '%s': %s", filename, details)
    ...
    if poster_url:
        try:
            ...
            logger.debug("Fetched poster for '%s'", filename)
    ...
    try:
        ...
        logger.info("Sent update for '%s' to channel %s", filename, MOVIE_UPDATE_CHANNEL)
